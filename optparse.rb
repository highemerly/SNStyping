...
class Option
  def initialize(argv=ARGV, mode)
    ...
    o.on('-l', '--limit VALUE', 'Specify limit of a number of contents per API page (default: 20)'){ |v| @opts[:limit] = v.to_i } if mode == "bookmark.rb" || mode == "favourite.rb"
  ...
  def command(filename, max_id)
    "ruby #{filename} -s #{@opts[:service]} -i #{@opts[:account_id]} -m #{max_id} -f #{@opts[:favourite_threshold]} #{@opts[:accept_unlisted_toot] ? "-u " : ""}-l #{@opts[:limit]} -n #{@opts[:num_of_page]}"
  end
end
