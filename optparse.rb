...
class Option
  def initialize(argv=ARGV, mode)
    ...
    o.on('-t', '--hashtag STRING', 'Specify hashtag (except #)') { |v| @opts[:hashtag] = v }
    ...
    o.on('-l', '--limit VALUE', 'Specify limit of a number of contents per API page (default: 20)'){ |v| @opts[:limit] = v.to_i }                                       if ["bookmark.rb", "favourite.rb"].include?(mode)
